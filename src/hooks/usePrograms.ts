...
import { Program, ProgramSection, ProgramFormData } from '../types/program';
...
export const usePrograms = () => {
        ...
        const sections = Array.isArray(row.program_sections)
            ? (row.program_sections as Record<string, unknown>[]).map(
                (s): ProgramSection => ({
                    ...
                    section_key: s.section_key as ProgramSection['section_key'],
    ...
    const deleteProgram = async (id: string): Promise<boolean> => {
        ...
        try {
            ...
            // Only owners can delete programs (programs table has no user_id column,
            // so we rely on role-based access: owner role can delete any program)
            if (userRole !== 'owner') {
                toast({
                    title: 'Error',
                    description: 'You can only delete your own programs',
                    variant: 'destructive',
                });
                return false;
            }
